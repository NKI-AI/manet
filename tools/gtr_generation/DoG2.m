function [dogFilterImage] = DoG2(grayImage)
...
B = std2(final_image);
T=2.5*B;
binary=imbinarize(final_image,T);
%binary = imbinarize(final_image, 'adaptive');
subplot(4, 2, 7); 
imshow(binary, []);
title('Binary Image', 'FontSize', fontSize);
[pixelCount_binary grayLevels_binary] = hist(binary(:));
subplot(4, 2, 8); 
bar(grayLevels_binary, pixelCount_binary);
grid on;
title('Histogram of Binary Image', 'FontSize', fontSize);
figure;
