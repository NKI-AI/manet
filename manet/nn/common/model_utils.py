...
def save_model(args, exp_dir, epoch, model, optimizer, lr_scheduler, name='model'):
    ...
    torch.save(save_dict, f=os.path.join(exp_dir, f'{name}_{epoch}.pt'))

    with open(os.path.join(exp_dir, 'last_model.txt'), 'w') as f:
        f.write(str(epoch))
...
def load_model(args, exp_dir, model, optimizer, lr_scheduler, name='model'):
    ...
    if args.resume:
        ...
        if os.path.exists(last_model_text_path):
            ...
            checkpoint_fn = os.path.join(exp_dir, f'{name}_{last_epoch}.pt')
